...
angular.module('firebase.utils', ['firebase', 'colorappApp.config'])
        .factory('fbutil', ['$window', 'FBURL', '$firebase', function($window, FBURL, $firebase) {
            ...
            return {
                ...
                syncArray: function(path, factoryConfig) {
                    console.info("syncArray path:"+path+" factoryConfig:"+factoryConfig+" arguments:"+arguments);
                    var syncResult =  syncData.apply(null, arguments).$asArray();
                    syncResult.$loaded().then(function() {
                        console.log("loaded record", syncResult);
                        angular.forEach(syncResult, function(data){
